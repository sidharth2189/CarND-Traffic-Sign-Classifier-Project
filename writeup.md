...
- Droput Introduced: This regularization technique was implemented for reducing overfitting, after the first and second fully connected layers with probabilities of keeping the units set at 50% for both.
- Filter Depth: The filter depth at the first and second convolutional layers were increased for better feature extraction. The 'k' value for first layer is 40 and second layer is 80.
* Which parameters were tuned? How were they adjusted and why?
- The batch size is limited to 128 since training the whole model at once is computationally expensive
- The number of epochs were set to 20 which leads to a good training accuracy and validation accuracy
- The learning rate is set 0.001. This could be reduced to see if the training accuracy fares any better over an increased number of epochs
- The dropout implemented after the first and second fully connected layers are set to 0.5 to reduce overfitting.
...
The neural network used the shape and edges of the content in the images to make classification as depicted in feature maps below. Following feature map sequence depicts what the network visualizes after the first convolution layer and activation function, with output size 28x28x40
